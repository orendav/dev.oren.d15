...
- Reference: `designs/gpt.v7/index.html`
...
- Drawer includes quick links for `T-Shirts`, `Polos`, `Hoodies`, and highlighted `Clearance`.
- Drawer uses a tabbed mobile mega-menu with these tabs: `Shop by Category`, `Shop by Industry`, `Shop by Gender`, `Shop by Services`.
...
- Quote request, contact form CTA, login/register, phone, and GST toggle remain accessible below the shop panels.
- Do not add duplicate bottom links for T-Shirts, Hoodies, Polos, On Sale, Help Centre, Find a Store, or plain Contact.
...
- Runtime images should use Imgix URLs directly. Local image fallbacks are intentionally removed in gpt.v7.
- The only local runtime assets expected in this folder are `images/activwear.mp4` and `images/favicon.svg`.
